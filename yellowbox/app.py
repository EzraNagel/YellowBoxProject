...
def kiosk_disks(kiosk_id):
    ...
    disks = db.session.query(Disk.id, Disk.condition, Movie.title).join(Movie, Disk.movieId == Movie.id).filter(
        Disk.location == kiosk_id).all()
...
def DVDs():
    ...
    disks = db.session.query(Disk.id, Disk.condition, Movie.title, Kiosk.address).join(Movie,
                                                                                       Disk.movieId == Movie.id).join(
        Kiosk, Disk.location == Kiosk.id).all()
